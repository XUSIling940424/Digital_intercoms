...
typedef struct
{
    uint32_t            stdID;         //标准标识符!< Specifies the standard identifier. It can be 0 to 0x7FF.
    uint32_t            extID;         //扩展帧标识符!< Specifies the extended identifier. It can be 0 to 0x1FFFFFFF.
    CAN_TYPEID_T        typeID;         //标识符类别
    CAN_RTXR_T          remoteTxReq;    //帧类型
    uint8_t             dataLengthCode;//数据长度!< Specifies the data length code.       It can be 0 to 8.
    uint8_t             data[8];       //具体数据!< Specifies the data to be transmitted. It can be 0 to 0xFF.
} CAN_TxMessage_T;
