...
//					rt_memcpy(my_can_tx_msg.data,my_send_buff_fifo.send_buff_fifo[my_send_buff_fifo.send_output],8);
					my_can_tx_msg.stdID=i | 0x0010;
//					my_can_tx_msg.extID=my_send_buff_fifo.send_output | 0x1000;
					my_can_tx_msg.remoteTxReq=CAN_RTXR_DATA;     //帧类型	
					my_can_tx_msg.dataLengthCode=8;  //数据长度		
					my_can_tx_msg.typeID=CAN_TYPEID_STD;          //标识符类别					
					ret=can1_send_msg(CAN1,&my_can_tx_msg);  
//					my_send_buff_fifo.send_output=(my_send_buff_fifo.send_output+1)%MAX_SEND_NUM;
				}
