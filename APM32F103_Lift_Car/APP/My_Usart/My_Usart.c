...
//		rt_kprintf("OK\n");
	...
	#if 1	
			/*can消息检测*/
			if(Recv_Can_buff[0].msg_output!=Recv_Can_buff[0].msg_input){  //第一路，机房语音
				My_Attribute.com_talk=1;   My_Attribute.com_talk_outtime=0;
...
//				rt_kprintf("send is num 1\n");
			  ...
			  My_Attribute.com_talk_outtime++;
				if(My_Attribute.com_talk_outtime>=MAX_TALK_TIMEOUT)   My_Attribute.com_talk=0;
			}	
			
		  if(Recv_Can_buff[1].msg_output!=Recv_Can_buff[1].msg_input){  //第二路，轿厢语音
				My_Attribute.lif_talk=1;  My_Attribute.lif_talk_outtime=0;
				rt_memset(my_usart_send,0,sizeof(my_usart_send));
				for(i=0;i<8;i++){
					my_usart_send[i*15]=0xef;    my_usart_send[i*15+1]=0xfa;
...
//				rt_kprintf("send is num 2\n");
			  ...
			  My_Attribute.lif_talk_outtime++;
				if(My_Attribute.lif_talk_outtime>=MAX_TALK_TIMEOUT)   My_Attribute.lif_talk=0;
			}
			
			
		 if(Recv_Can_buff[2].msg_output!=Recv_Can_buff[2].msg_input){   //第三路，底坑语音
			  My_Attribute.pit_talk=1;  My_Attribute.pit_talk_outtime=0;
				rt_memset(my_usart_send,0,sizeof(my_usart_send));
				for(i=0;i<8;i++){
					my_usart_send[i*15]=0xef;    my_usart_send[i*15+1]=0xfa;
...
//				rt_kprintf("send is num 3\n");
			  ...
			  My_Attribute.pit_talk_outtime++;
				if(My_Attribute.pit_talk_outtime>=MAX_TALK_TIMEOUT)   My_Attribute.pit_talk=0;
			}	
		 
		if(Recv_Can_buff[3].msg_output!=Recv_Can_buff[3].msg_input){   //第四路，轿顶语音
			  My_Attribute.car_talk=1;  My_Attribute.car_talk_outtime=0;
				rt_memset(my_usart_send,0,sizeof(my_usart_send));
				for(i=0;i<8;i++){
					my_usart_send[i*15]=0xef;    my_usart_send[i*15+1]=0xfa;
...
//				rt_kprintf("send is num 4\n");
			  ...
			  My_Attribute.car_talk_outtime++;  
				if(My_Attribute.car_talk_outtime>=MAX_TALK_TIMEOUT)  My_Attribute.car_talk=0;
			}
		
		if(Recv_Can_buff[4].msg_output!=Recv_Can_buff[4].msg_input){   //第五路，值班室语音
			  My_Attribute.dut_talk=1;  My_Attribute.dut_talk_outtime=0;
				rt_memset(my_usart_send,0,sizeof(my_usart_send));
				for(i=0;i<8;i++){
					my_usart_send[i*15]=0xef;    my_usart_send[i*15+1]=0xfa;
					my_usart_send[i*15+2]=0x08;  my_usart_send[i*15+3]=0x0b;  my_usart_send[i*15+4]=0x02;			
...
//				rt_kprintf("send is num 5\n");
				usart2_dma_send(my_usart_send,120);
				Recv_Can_buff[4].msg_output=((Recv_Can_buff[4].msg_output+1)%MAX_MSG_NUM);
			}else{
			  My_Attribute.dut_talk_outtime++;
				if(My_Attribute.dut_talk_outtime>=MAX_TALK_TIMEOUT)  My_Attribute.dut_talk=0;
			}		
	#endif			 			
			}else{
				;
