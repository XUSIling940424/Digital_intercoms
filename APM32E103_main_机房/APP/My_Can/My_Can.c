  ...
  my_can_tx_msg.extID=0x00420000;  //扩展帧标识符
	...
	/*等待发送信号量*/
		...
		if(result != RT_EOK){   /*信号量未释放*/
          ...
          rt_memcpy(&can2_send_buff.msg[can2_send_buff.input],&my_can_tx_msg,sizeof(my_can_tx_msg));		//拷贝到can2发送区			
...
static void Can2_Send_thread_entry(void *parameter)
{
  ...
  while(1){
	/*等待发送信号量*/
      ...
      if(my_can2_send_remote_msg.Msg_In!=my_can2_send_remote_msg.Msg_Out){   /*有消息需要发送*/
				my_can2_tx_msg.extID=my_can2_send_remote_msg.Can_Msg[my_can2_send_remote_msg.Msg_Out].id;
				my_can2_tx_msg.remoteTxReq=CAN_RTXR_REMOTE;     //帧类型
				my_can2_tx_msg.typeID=CAN_TYPEID_EXT;          //标识符类别
