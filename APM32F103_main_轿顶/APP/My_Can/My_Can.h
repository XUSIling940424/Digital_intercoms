...
#define   Tall_State_DUTH     0x04     //和值班室通话状态
#define   Call_State_COVER    0x05     //被呼叫状态

struct Attribute_t{
  unsigned char state;      //状态
	unsigned char addr;       //地址
	unsigned char com_talk;   //机房通话标志
	unsigned char com_talk_outtime;//机房超时未接收到语音数据
	unsigned char dut_talk;   //值班室通话标志	
	unsigned char dut_talk_outtime;//值班室超时未接收到语音数据
	unsigned char car_talk;   //轿顶通话标志
	unsigned char car_talk_outtime;//轿顶超时未接收到语音数据
	unsigned char lif_talk;   //轿厢通话标志
	unsigned char lif_talk_outtime;//轿厢超时未接收到语音数据
	unsigned char pit_talk;   //底坑通话标志	
	unsigned char pit_talk_outtime;//底坑超时未接收到语音数据
};
